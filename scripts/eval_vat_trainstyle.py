...
def main():
    ...
    vat_eval_dataset = GazeDataset(['videoattentiontarget'], [args.data_path], 'test', transforms, head_transforms, in_frame_only=False, sample_rates=[1])
    ...
    for batch in tqdm(vat_eval_dl, "Evaluating VAT (train-style)", len(vat_eval_dl)):
    ...
    plt.tight_layout()
    plt.savefig('./visualization/hist_vat.png', dpi=200, bbox_inches="tight")
    plt.close()
