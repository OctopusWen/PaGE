...
def main():
    ...
    gooreal_eval_dataset = GazeDataset(['gooreal'], [args.data_path], 'test', transforms, head_transforms, in_frame_only=False, sample_rates=[1])
    gooreal_eval_dl = torch.utils.data.DataLoader(
        gooreal_eval_dataset, 
    ...
    for batch in tqdm(gooreal_eval_dl, "Evaluating GOOReal (train-style)", len(gooreal_eval_dl)):
    ...
    plt.tight_layout()
    plt.savefig('./visualization/hist_gooreal.png', dpi=200, bbox_inches="tight")
    plt.close()
